...
var resizeId;
var windowWidth = window.innerWidth; //staring
var totalRoutes = 0;
...
//lang button
d3.select("#lang-btn").text(function() {
    if(window.innerWidth<750) return params.labels.switchLangShort[lang];
        else return params.labels.switchLang[lang];
    }
).on('click', function() {
...
ymaps.ready(function () {
    ...
    var timerID;
    var demoTimer; //timer for demo mode
    var demoID = 0; //station code for demo mode
    ...
    var routesMap = new ymaps.Map("map-routes", mapParams),
    ...
    //placemark templates
    var circleLayout = ymaps.templateLayoutFactory.createClass('<div style="width: {{properties.radius2x}}px; height: {{properties.radius2x}}px; display: block; position: relative; top: -{{properties.radius}}px; left: -{{properties.radius}}px; border-radius: {{properties.radius}}px; background: {{properties.color}}; opacity: {{properties.opacity}};"></div>'),
        heatMapLabel = ymaps.templateLayoutFactory.createClass('<div style="width: 30px; height: 12px; display: block; font-size: 10px; color: #cccccc;">{{properties.weight}}</div>');
    ...
    var maxValues = {
        bestDay: { //the best value of rents per day
            value: 0,
            date: ""
        },
        bestHour: { //the best value of rents per day
            value: 0,
            date: ""
        },
        bestStation: {
    ...
    var stationsRoutes = new ymaps.GeoObjectCollection(null, {
        opacity: 0.5
    }, {
    ...
    stationsRoutes.events
        ...
        .add('mousemove',  function(e) {
            getTooltip(event.pageX, event.pageY, {
                ...
                second: e.get('target').properties.get('total') + " (" + Math.round(e.get('target').properties.get('total')/stations[e.get('target').properties.get('to')].total*100*100)/100 + "%)"

            });
        });


    var stationsPlacemarks = new ymaps.GeoObjectCollection(null, {  });
    var stationsPlacemarksHeatmap = new ymaps.GeoObjectCollection(null, {  });


    var routes = [], routesOrdered = [],
        stations = [], stationsOrdered = [],
        activities = [], activitiesTotals = [], activitiesTotalsDaily = []; //array with activities on map^ totals — ordered array fot path

    var weather = []; //array for weather conditions
    ...
    var dateFormat = d3.time.format("%Y%m%d%H"),
        ...
        endDate = dateFormat.parse("2014110100"),
    ...
    var leftOffset = (window.innerWidth - tickWidth * allDays) / 2, //the space left/right
    ...
    var routeDescriptionTemplate = ymaps.templateLayoutFactory.createClass('<div class="route-in-balloon"><div class="route-total" title="общее число прокатов">{{properties.total}}</div><div class="route-title">{{properties.nameTo}}</div><div class="route-params"><span class="route-code">#{{properties.to}}</span><span class="route-duration">{{properties.duration}} мин.</span><span class="route-distance">{{properties.distance}}</span></div></div>');
    ...
    var svgAreaBg = svgArea.append("g"),
        svgAreaAxis = svgArea.append("g").attr("transform", "translate(0,60)"),
        svgPointer = svgArea.append("rect")
            .attr("height", 80).attr("width", 2)
            .style("fill", "#ff0000")
            .attr("x", 0)
            .attr("y", 0);

    //button-menu for small-screens
    d3.select("#menu-mode").on("click", function() {
        toggleMenu();
    });

    //button heatmap-filters
    d3.select("#heatmap-filter-icon").on("click", function() {
        heatmapFilter(true);
    });


    var heatmapInfo = d3.select("#heatmap-info"),
        ...
        heatmapInfoScoreDescription = d3.select("#heatmap-info-score-description").text(params.labels.rentsPerHour[lang]);

    //binding clicks on menu items
    d3.select("#mode-routes").text(params.labels.menuRoutes[lang]).on('click', function () {
        changeMode('routes');
    });
    d3.select("#mode-heatmap").text(params.labels.menuHeatmap[lang]).on('click', function () {
        changeMode('heatmap');
    });
    d3.select("#mode-calendar").text(params.labels.menuCalendar[lang]).on('click', function () {
        changeMode('calendar');
    });
    d3.select("#mode-about").text(params.labels.menuAbout[lang]).on('click', function () {
        changeMode('about');
    });

    d3.select("#station-panel-close").on('click', function () {
        togglePanel(false);
    });
    ...
    function importRentData() {
        ...
        d3.tsv(routesDataFilename, function (d) {
            ...
            function () {
                ...
                for (var r in routes) routesOrdered.push(routes[r]);
                ...
                for (var m in stations) {
                    ...
                    for (var w in stations) {
                        ...
                        if (routes[rt]) stations[m].routes.push(
                            {
                                to: routes[rt].to,
                ...
                stationsOrdered.sort(function (a, b) {
                ...
                routesOrdered.sort(function (a, b) {
                    return a.total == b.total ? 0 : +(a.total < b.total) || -1;
...
//                console.log(maxValues);
    ...
    function toggleMenu() {
        if(window.innerWidth<750) {
            var menuState = d3.select("#menu").style("visibility");
            if (menuState == 'hidden') {
...
//the function gets the panel content
    function getStationPanel(code) {
        ...
        var s = getStationIndex(code);
        stationPanelContent.text("");
        stationPanelName.text("");
        stationPanelName.append("div")
        ...
        var days = (endDate.getTime() - stations[code].start.getTime())/(1000*60*60*24); //how many days station worked
        var maxDay = d3.max(stations[code].byDay);
        var maxHour = d3.max(stations[code].byHour);
        ...
        d3.select("#station-total-perday-value").text(Math.round(stations[code].total/days*10)/10);
        d3.select("#station-total-perday-description").text(params.labels.rentsPerDay[lang]);

        d3.select("#station-total-percent-value").text(Math.round(stations[code].total/totalRoutes*1000)/10);
...
*/
        ...
        if((window.innerWidth<700) || window.innerHeight<400) {
            ...
            //adding routes
            stationPanelContent.append("div").attr("class", "station-subtitle").text(params.labels.activitiesByDay[lang]);
            var graphByDays = stationPanelContent.append("svg").attr("width", 300).attr("height", 100);
            ...
            var yAxisDay = d3.svg.axis()
                .scale(y)
                .orient("left");


            params.days.forEach(function (d, i) {
                graphByDays
                    .append("rect")
                    .attr("width", 40)
                    .attr("height", function () {
                        return (stations[code].byDay[d.num] / maxDay * 80);
                    })
                    .attr("x", 42 * i)
                    .attr("y", function () {
                        return (80 - stations[code].byDay[d.num] / maxDay * 80);
                    })
                    .style("fill", params.colors.base)
                    .style("opacity", 0.6)
            ...
            stations[code].byHour.forEach(function (d, i) {
                graphByHours
                    .append("rect")
                    .attr("width", 11)
                    .attr("height", function () {
        ...
        stations[code].routes.forEach(function (d, i) {
                ...
                if (stations[code].code !== d.to) {
                    stationsRoutes.add(new ymaps.Polyline([
                        ...
                        [stationsOrdered[getStationIndex(d.to)].lat, stationsOrdered[getStationIndex(d.to)].lon]
                    ], {
    ...
    function mapStations() {
        ...
        stationsOrdered.forEach(function (station,i) {
            var w = 10; //default minimum size of station point
            w = Math.floor(Math.sqrt(station.total*scale)/3);

            if (w > 0) {
                var pl = new ymaps.Placemark([station.lat, station.lon],
                    {
                        code: station.code,
...
//                });
    ...
    function colorStations(mode,code) {
        ...
        stationsPlacemarks.each(function(item) {
            ...
            if(mode == 'months') {
                var month = stations[item.properties.get('code')].start.getMonth();
    ...
    function buildTimeScale() {
        ...
        d3.select("#total-heatmap").text(params.labels.totalHeatmap[lang]).on("click", function() {
            heatMapStations();
        });

        d3.select("#heatmap-controls-days").append("div").attr("class", "control-description").text(params.labels.hcDays[lang]);
        ...
        d3.select("#heatmap-controls-hours").append("div").attr("class", "control-description").text(params.labels.hcHours[lang]);

        //adding control by hour
        for (var h = 0; h < 12; h++) {

            d3.select("#heatmap-controls-hours")
                .append("div")
                .attr("class", "control")
                .attr("id", "byHour-" + (h * 2))
                .text((h * 2) + ":00")
...
//function converts pixelt to date
    ...
    function humanDate(date, short) {
        if (short)
            return (date.getDate() + " " + params.months[date.getMonth()].short[lang]);
        else
            var d = date.getDay();
            d = d<1 ? 6 : d-1;
            return (date.getDate() + " " + params.months[date.getMonth()].long[lang] + " " + params.days[d][lang]);
    }
    ...
    function heatMapActivity(dt) {
        ...
        if (activities[dt]) {
            ...
            heatmapInfoWeather.text(weather[dateFormatDay(currentDate)].temp + "°C, " + weather[dateFormatDay(currentDate)].description[lang]);
    ...
    function heatMapStations(mode) {
        ...
        stationsOrdered.forEach(function (d, i) {
...
//        hotMap.geoObjects.add(stationsPlacemarksHeatmap);
    ...
    function buildCalendar() {
        ...
        var activitySvg = d3.select("#calendar-content").append("svg").attr("width", window.innerWidth).attr("height", stationsOrdered.length*16+320),
            activityGraphYAxis = activitySvg.append("g"),
            activityGraphYAxisWeather = activitySvg.append("g"),
            activityGraphBars = activitySvg.append("g").attr("transform", "translate(" + leftOffset+",0)"),
        ...
        var activityDaily = [];
        for(var av in activitiesTotalsDaily) {
        ...
        activityDaily.sort(function(a,b) { return(a.x- b.x); });
...
*/
        ...
        var yScaleWeather = d3.scale.linear().range([250, 0]).domain([-10,35]),
            ...
            .on("mousemove", function(z){
                ...
                getTooltip(event.pageX, event.pageY,
                    { first: humanDate(pixelsToDate(d3.mouse(this)[0],tickWidth*allDays)),
                      second: activitiesTotalsDaily[dateFormatDay(pixelsToDate(d3.mouse(this)[0],tickWidth*allDays))],
                      third: weather[dateFormatDay(pixelsToDate(d3.mouse(this)[0],tickWidth*allDays))].temp + "°C, " +
                          weather[dateFormatDay(pixelsToDate(d3.mouse(this)[0],tickWidth*allDays))].description[lang]
                    });
                activityGraphPointer
        ...
        var calendarTimeScaleArea  = calendarTimeScaleSvg
            ...
            .attr("height", 320);
        var calendarContentArea  = activitySvg.append("g").attr("transform", "translate(0,285)");
        ...
        var calendarContentSvgTimeScale = activitySvg.append("g").attr("transform", "translate (" + leftOffset + "," + (16*stationsOrdered.length+290) + ")");
            ...
            if(station.start) {
                ...
                for(var dt in station.byDate) {
                    ...
                    row.append("rect")
                        ...
                        //.attr("opacity",1)
                        .attr("x", function() {
                            var x = Math.floor((ddd.getTime() - startDate.getTime())/(1000*60*60*24)*tickWidth);
                            return x+leftOffset;
                        })
